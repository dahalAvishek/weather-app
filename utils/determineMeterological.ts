...
export const calculateAirPressure = (elevation: number): number => {
  const airPressure =
    P0 * Math.pow(1 - (L * elevation) / T0, (g * M) / (R * L));
  return airPressure;
};

export const calculateFeelsLike = (currentData: SelectedData): number => {
  const feelsLike =
    c1 +
    ...
    c7 *
      ...
      currentData.temperature_2m *
      currentData.relativehumidity_2m +
    c8 *
      currentData.temperature_2m *
      currentData.relativehumidity_2m *
      currentData.relativehumidity_2m +
...
const calculateRainfallChances = (currentData: SelectedData): number => {
