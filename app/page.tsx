...
export default function Home() {
  ...
  const [selectedTime, setSelectedTime] = useState<string>(getFormattedDate(new Date()));
    ...
    <main className="main-wrapper p-8">
      {data && (
        <div className="h-full flex justify-between flex-wrap ">
          ...
          <Meteorological hourly={data.hourly} elevation={data.elevation} selectedTime={selectedTime}/>
          <DailyTemperatureCard hourly={data.hourly} selectedTime={selectedTime} setSelectedTime={setSelectedTime}/>
        </div>
      )}
    </main>
  );
